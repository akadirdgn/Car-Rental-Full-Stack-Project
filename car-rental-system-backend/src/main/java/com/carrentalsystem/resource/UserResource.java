...
public class UserResource {
	...
	public ResponseEntity<CommonApiResponse> registerAdmin(RegisterUserRequestDto registerRequest) {
		...
		if (registerRequest == null) {
			response.setResponseMessage("Geçersiz istek - kullanıcı boş olamaz.");
	...
	public ResponseEntity<CommonApiResponse> registerUser(RegisterUserRequestDto request) {
		...
		if (request.getRole() == null) {
			response.setResponseMessage("Geçersiz istek - rol bilgisi eksik.");
			response.setSuccess(false);
	...
	public ResponseEntity<UserResponseDto> getUsersByRole(String role) {
		...
		response.setResponseMessage("Kullanıcılar başarıyla getirildi.");
		response.setSuccess(true);
	...
	public ResponseEntity<CommonApiResponse> updateUserStatus(UserStatusUpdateRequestDto request) {
		...
		if (request == null) {
			response.setResponseMessage("Geçersiz istek - veri eksik.");
			response.setSuccess(false);
		...
		if (request.getUserId() == 0) {
			response.setResponseMessage("Geçersiz istek - kullanıcı ID eksik.");
			response.setSuccess(false);
		...
		response.setResponseMessage("Kullanıcı durumu başarıyla güncellendi.");
		response.setSuccess(true);
		return new ResponseEntity<CommonApiResponse>(response, HttpStatus.OK);
	...
	public ResponseEntity<CommonApiResponse> addCustomerDrivingLicense(AddDrivingLicenseRequest request) {
		...
		response.setResponseMessage("Sürücü belgesi başarıyla eklendi.");
		response.setSuccess(true);
